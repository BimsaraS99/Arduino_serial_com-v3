...
def Find_Edge(image_send):
    try:
        ...
        line_distance = height // 20
        for i in range(1, 20):
            y = i * line_distance
            cv2.line(result, (0, y), (result.shape[1], y), (0, 0, 255), thickness=1)
        ...
        for contour in contours:
            if cv2.contourArea(contour) >= 0 and cv2.contourArea(contour) <= shadow_size:
                cv2.drawContours(contour_img, [contour], 0, (255, 255, 255), -1)
        cv2.imshow('Bimsara', contour_img)

        edges = cv2.Canny(contour_img, 50, 150, apertureSize=3)
        lines = cv2.HoughLines(contour_img, 1, np.pi / 180, 100)
        ...
        for i in range(10):
            ...
            angle_ = angle_to_x_axis(x1, y1, x2, y2)
            print(angle_)
            if angle_ < 0:
                angle_ = angle_ * -1

            if angle_ > line_angle:
                pre_val[0], pre_val[1], pre_val[2], pre_val[3] = x1, y1, x2, y2
                cv2.line(image, (x1, y1), (x2, y2), (255, 255, 0), 2, cv2.LINE_AA)
                return image, (x1, y1), (x2, y2)
            else:
                print("fk")
                cv2.line(image, (pre_val[0], pre_val[1]), (pre_val[2], pre_val[3]), (255, 255, 0), 2, cv2.LINE_AA)
                return image, (pre_val[0], pre_val[1]), (pre_val[2], pre_val[3])
...
#cap = cv2.VideoCapture('A:/Internship MAS/03.03.2023/edge detector dataset/video8.mp4')
cap = cv2.VideoCapture(1)
arduino = serial.Serial('COM15', 9600, timeout=1)  # 23.3.2023
time.sleep(2)  # 23.3.2023
...
while True:
    try:
        ...
        draw_dashed_line(return_image, 0, (height // 5) * 4, width + 20, (height // 5) * 4, (0, 0, 0), 1, 10,
                         cv2.LINE_AA)

        intersection_point_1 = get_intersection_point((coordinate_1 + coordinate_2),
                                                      (0, height // 5, width, height // 5))
        cv2.circle(return_image, intersection_point_1, 10, (0, 0, 255), thickness=2)

        intersection_point_2 = get_intersection_point((coordinate_1 + coordinate_2),
                                                      (0, (height // 5) * 4, width + 20, (height // 5) * 4))
        ...
        cv2.putText(space, "X = " + str(intersection_point_1[0] - (width // 2)), (20, 35), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (0, 0, 255), 2)
        cv2.putText(space, "X = " + str(intersection_point_2[0] - (width // 2)), (440, 35), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (255, 0, 0), 2)

        sending_value = str(intersection_point_1[0] - (width // 2)) + '+' + str(intersection_point_2[0] - (width // 2))
        arduino.write(sending_value.encode())
